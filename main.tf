...
# Terraform will create the inventory.ini file automatically
  ...
  command = <<EOT
    ...
    echo "${self.public_ip} ansible_user=ec2-user ansible_ssh_private_key_file=/var/lib/jenkins/.ssh/ansible-key" >> /var/lib/jenkins/workspace/Terraform-Ansible-Pipeline/inventory.ini
  EOT
}
...
# User data for setting up SSH keys
  user_data = <<-EOF
    #!/bin/bash
    # Add SSH public key to authorized_keys to allow Ansible login
    mkdir -p /home/ec2-user/.ssh
    echo "ssh-rsa  ec2-user@ip-172-31-10-241.eu-west-1.compute.internal" > /home/ec2-user/.ssh/authorized_keys
